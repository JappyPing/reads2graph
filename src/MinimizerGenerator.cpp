...
{   
    ...
    if (args.default_params) {
        ...
        if (args.segmentation && args.read_length >= 16) {
            uint8_t segment_size = static_cast<uint8_t>(std::ceil(args.read_length / num_substr));   
            if (args.bucketing_mode == "minimizer_gomh") {
                k_size = k_estimate(num_substr);
                w_size = static_cast<uint8_t>(segment_size * args.alpha);  
            } else if (args.bucketing_mode == "miniception_gomh"){
                k_size = static_cast<uint8_t>(std::ceil((2 * (segment_size + 1) - std::ceil(args.n_kmer/num_substr)) / (std::ceil(args.n_kmer / num_substr) * args.beta + 2)));
                if (k_size < 3){
                    k_size = 3;
                }  
                w_size = static_cast<uint8_t>(std::ceil(k_size * args.beta));
                if (w_size >= segment_size - k_size + 1) {
                    w_size = k_size + 1; 
                }  
            }          
        } else {
...
//     uint8_t k = static_cast<uint8_t>(ceil((args.differ_kmer_ratio * (1 + segment_size))/(1 + args.differ_kmer_ratio)));
//     if (k > 28) {
//         k = 28;       
//     } else if (k < 4){
//         k = 4;       
...
//     auto uni_reads_num = unique_reads.size();
//     Utils::getInstance().logger(LOG_LEVEL_INFO,  std::format("The number of unique reads: {} ", uni_reads_num));
//     std::size_t sample_num = static_cast<std::size_t>(p * uni_reads_num);

//     // Set up a default random number generator
//     std::default_random_engine gen;

//     // Create an index vector to randomly shuffle
//     std::vector<std::size_t> indices(uni_reads_num);
